...
- `hdl-helper.targetDrivenRuns.enabled`
  ...
  - Use `HDL: Run Active Target Simulation` to resolve active target top and run simulation directly (with visible heuristic fallback warning when config top is unresolved).
